// Workflow that mirrors one Protomaps daily PMTiles archive into R2.
// The upstream archives at build.protomaps.com expire after roughly a
// week, so by snapshotting a build into R2 once a month we get a
// stable, long-lived copy and stop depending on the upstream's
// retention.
//
...
export class PmtilesMirrorWorkflow extends WorkflowEntrypoint<Env, PmtilesMirrorParams> {
  override async run(event: WorkflowEvent<PmtilesMirrorParams>, step: WorkflowStep): Promise<unknown> {
    ...
    // Pointer file the tile worker reads at request time. Tiny enough
    // that a get-per-instance is cheap; we still memoize on the read
    // side with an hourly TTL.
